...
export default function BalanceSheetPage() {
  ...
  async function fetchBalanceSheet(saveSnapshot = false) {
    ...
    try {
      ...
      const res = await fetch(`/api/accounts/balance-sheet?${params.toString()}`, {
        cache: "no-store",
      });
  ...
  function exportCsv() {
    ...
    const rows = [
      ...
      ...data.assets.map((x) => ["Assets", x.code, x.name, x.debit, x.credit, x.amount, x.warning || ""]),
      ...data.liabilities.map((x) => ["Liabilities", x.code, x.name, x.debit, x.credit, x.amount, x.warning || ""]),
      ...data.equity.map((x) => ["Equity", x.code, x.name, x.debit, x.credit, x.amount, x.warning || ""]),
    ];
      ...
      <div className="mx-auto max-w-7xl space-y-5 print:max-w-none print:space-y-3">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl print:border-black/20 print:bg-white print:p-3 print:shadow-none">
          ...
          <p className="mt-2 text-sm text-slate-400 print:text-xs print:text-black">
            Unified Finance Suite: shared filters, AI solvency, monthly comparison, risk heat and ledger drill-down.
          </p>
        </div>
        ...
        {data && (
          <>
            <div className="grid gap-4 md:grid-cols-4 print:grid-cols-4 print:gap-2">
              ...
              <Stat title="Liabilities" value={money(data.summary.liabilities)} />
              <Stat title="Equity / Income" value={money(data.summary.equity)} />
            ...
            <div className="rounded-3xl border border-indigo-400/20 bg-gradient-to-r from-indigo-950 to-slate-900 p-6 shadow-2xl print:border-black/20 print:bg-white print:p-3 print:shadow-none">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm print:border-black/20 print:p-2 print:text-xs">
                  <p>Asset/Liability Ratio: <b>{data.summary.assetToLiabilityRatio}</b></p>
                  <p>Debt Ratio: <b>{data.summary.debtRatio}%</b></p>
                  <p>
                    Balance Status:{" "}
                    <b className={data.summary.balanced ? "text-emerald-300 print:text-black" : "text-red-300 print:text-black"}>
                      {data.summary.balanced ? "Balanced" : "Not Balanced"}
                    </b>
                  </p>
                  <p>Difference: <b>{money(data.summary.difference)}</b></p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <h3 className="text-lg font-black print:text-sm">AI Management Action Plan</h3>
            ...
            <div className="grid gap-6 lg:grid-cols-3 print:grid-cols-3 print:gap-2">
              <SectionCard title="Assets" rows={data.assets} total={data.summary.assets} onRowClick={openLedger} />
              <SectionCard title="Liabilities" rows={data.liabilities} total={data.summary.liabilities} onRowClick={openLedger} />
              <SectionCard title="Equity / Income" rows={data.equity} total={data.summary.equity} onRowClick={openLedger} />
            </div>
          </>
        )}
      </div>
      ...
      <div className="space-y-2 print:space-y-1">
          ...
          rows.slice(0, 18).map((row) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <p className={row.amount < 0 ? "font-black text-red-300 print:text-[10px] print:text-black" : "font-black print:text-[10px]"}>
                  {money(row.amount)}
                </p>
              </div>
            </button>
        ...
        {rows.length > 18 && (
          <p className="pt-2 text-xs font-bold text-slate-400 print:text-[9px] print:text-black">
            + {rows.length - 18} more accounts. See ledger drill-down for full detail.
          </p>
        )}
      </div>
    </div>
