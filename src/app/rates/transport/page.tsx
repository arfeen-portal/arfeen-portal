...
export default function TransportRateRulesPage() {
  ...
  async function loadRules() {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/rates/rules/list?service=transport");
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3">
        <h1 className="text-2xl font-semibold">Transport Rate Rules</h1>
        <button
          type="button"
          onClick={startNew}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="JED-MAKKAH Hiace 0-120km"
              required
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="use_flat" className="text-sm">
              Use flat rate (ignore per km)
            </label>
          </div>
            ...
            {!loading &&
              rules.map((r) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-3 py-2">
                    {r.min_distance_km ?? "0"} – {r.max_distance_km ?? "∞"} km
                  </td>
                  <td className="px-3 py-2">
                    {r.use_flat
                      ? `Flat SAR ${r.base_flat ?? 0}`
                      : `SAR ${r.base_per_km ?? 0}/km`}
                  </td>
                  <td className="px-3 py-2">
