...
function applyTenantScope(query: any, tenantId: string | null, role: string) {
  if (tenantId) return query.eq("tenant_id", tenantId);
  if (canViewGlobal(role)) return query;
  throw new Error("Tenant not assigned to this user.");
}
...
function mustHaveTenantForWrite(role: string, tenantId: string | null) {
  ...
  if (role === "super_admin" || role === "admin") {
    return "tenant_id is missing from your user profile. Select/create a tenant before changing invoices.";
  }
...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    if (delErr) {
      return jsonError(delErr.message || "Failed to clear invoice lines", 500);
    }
...
export async function DELETE(req: Request, context: RouteContext) {
  try {
    ...
    return jsonOk({ deleted: true, tenant_id: tenantId });
  } catch (error) {
    return jsonError(
      error instanceof Error ? error.message : "Unauthorized.",
      401
