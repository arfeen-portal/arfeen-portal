...
function recoveryAction(row: any, score: number, brokenPromises: number) {
  if (brokenPromises >= 3) {
    return {
      ...
      ai_note:
        "Habitual broken promises detected. Legal review and blacklist recommendation triggered.",
    };
  }
  ...
  if (row.aging_bucket === "90_plus" || score < 35) {
    return {
      ...
      ai_note:
        "High default probability. Stop further credit and collect written payment commitment.",
    };
  }
  ...
  if (row.aging_bucket === "61_90" || n(row.balance_amount) > 500000) {
    return {
      ...
      ai_note:
        "Secure partial payment first, then convert remaining balance into dated commitment.",
    };
  }
  ...
  if (row.aging_bucket === "31_60") {
    return {
      ...
      ai_note:
        "Medium risk. Relationship-friendly reminder is suitable before escalation.",
    };
  }
...
export async function GET(req: Request) {
  try {
    ...
    if (tenantId) promisesQuery = promisesQuery.eq("tenant_id", tenantId);
    ...
    let aging = (data || []).map((row: any) => {
      ...
      const recovery_probability = Math.max(
        3,
        propensity_score - Math.round(n(row.overdue_days) * 0.15)
      );
    ...
    if (riskFilter !== "all") {
      aging = aging.filter((r: any) => r.risk_level === riskFilter);
    }
