...
export async function POST(req: NextRequest) {
  const { batchId, threshold = 0.82 } = await req.json();
  ...
  const { data: stagingRows, error: stagingError } = await supabaseAdmin
  ...
  if (updates.length > 0) {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to apply matches' }, { status: 500 });
    }
  }

  // audit log (user_id null for now)
  await supabaseAdmin.from('agent_import_audit_log').insert({
    tenant_id: null,
    user_id: null,
    batch_id: batchId,
    action: 'auto_match',
    meta: { matched: updates.length, total: stagingRows.length, threshold },
  });
